...
horror_new2 |>
  ...
  ggplot(aes(x = prop_profit)) +
  geom_density() +
  facet_wrap(. ~ genre_names, ncol = 1) +
  coord_cartesian(xlim = c(0, 100))

horror_new2 |>
  filter(genre_names %in% popular_genre_names$genre_names) |>
  mutate(en_language = ifelse(original_language == "en", "en", "non-en")) |>
  mutate(profitability = ifelse(adj_profit2020 > 0, "Profitable", "Unprofitable")) |>
  ggplot(aes(y = genre_names, fill = profitability)) +
