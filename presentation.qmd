...
How have trends in factors surrounding horror as a genre (e.g. popularity, budgets, etc) have changed over time?
...
p1 <- horror_summary |>
  mutate(total_budget = total_budget / 1000000) |>
  mutate(total_profit = total_profit / 1000000) |>
  ...
  labs(
    #title = "Starting in 2007, Movie Directors realized they didn't need big #budgets to garner an audience for their movies",
       y = "Dollars (2020 adjusted, millions)",
...
horror_new2_1 <- horror_new2 |>
  ...
  mutate(profit = factor(profit, levels = c("Very Profitable", "Profitable", "Unprofitable")),
         genre_names = factor(genre_names, levels = c("Horror", "Action", "Science Fiction", "Drama", "Thriller", "Mystery")))
...
## Q2: Sentiment Analysis by Subgenre with AFINN
...
## Q2: Sentiment Analysis by Subgenre with BING
...
## Q2: Budgets by Subgenre

```{r}
horror_new2 |>
  filter(genre_names %in% popular_genre_names$genre_names) |>
  filter(genre_names != "Horror") |>
  mutate(adj_budget2020 = adj_profit2020 / 1000000) |>
  ggplot(aes(x = genre_names,
             y = adj_budget2020,
             fill = genre_names)) +
  geom_boxplot() + 
  coord_flip(ylim = c(0, 120)) +
  scale_fill_manual(values = c("steelblue3","steelblue1","dodgerblue","cornflowerblue","lightskyblue")) +
  theme(legend.position = "none") +
  labs(x = "Subgenre",
       y = "Dollars (2020 Adjusted, by millions)",
       title = "Budgets for Horror Films",
       subtitle = "By Subgenre")
```
