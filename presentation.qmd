...
description1 <-
  "The horrific events of 9/11 got people on edge. The political uncertainty of the time increased the popularity of horror movies that allowed people to have an outlet for their anxiety." |> 
  str_wrap(width=40)

description2 <-
  "The stock market crashes in 2009. Horror films reach their peak popularity due to uncertainty and anxiety about the economic state of the US." |> 
  str_wrap(width=40)
...
horror_new |>
  ...
  geom_point(aes(col = "Total Budget")) +
  geom_line(aes(col = "Total Budget")) +
  geom_smooth(se = FALSE, aes(col = "Total Budget")) +
  geom_point(aes(y = total_profit, col = "Total Profit")) +
  geom_line(aes(y = total_profit, col = "Total Profit")) +
  geom_smooth(aes(y = total_profit, col = "Total Profit"), se = FALSE) +
  ...
  theme(legend.key.size = unit(3, 'cm'), #change legend key size
        ...
        axis.title=element_text(size=14)) +
  labs(title = "Starting in 2007, Movie Directors realized they didn't need big budgets to garner an audience for their movies",
       y = "$ adj. for inflation",
       x = "Year",
