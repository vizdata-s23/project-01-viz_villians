...
p1 <- horror_summary |>
  ...
  geom_line(aes(col = "Total Budget"), alpha=0.1, linewidth=2) +
  geom_smooth(se = FALSE, aes(col = "Total Budget"), linewidth=4, span=0.4) +
  geom_line(aes(y = total_profit, col="Total Profit"), alpha=0.1, linewidth=2) +
  geom_smooth(aes(y = total_profit, col = "Total Profit"),
              se = FALSE, linewidth=4, span=0.4) +
  scale_color_manual(name='',
                     breaks=c('Total Profit', 'Total Budget'),
                     values=c('Total Profit'="#d95f02", 'Total Budget'="#7570b3")) +
  ...
  labs(y = "Dollars (in billions)",
       title = "Total profits trended up from the late 1990s to the pre-COVID 2010s, while total budgets stagnated")
  



p2 <- horror_summary |>
  mutate(max = ifelse(release_year == 2009, "1", "0")) |>
  ggplot(aes(x = release_year, y = count, fill =max)) +
  geom_col() +
  theme_minimal()  +
  scale_fill_manual(values=c("darkgrey", "#1b9e77"))+
  labs(y = "Number of movies",
       x = "Year",
       caption = "Note: Dollars are 2020 inflation adjusted") + 
  coord_cartesian(xlim = c(1980 ,2022)) +
